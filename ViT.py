...
class PrototypicalNetwork(nn.Module):
    def __init__(self, model_name: str = 'vit_small_patch16_224', pretrained: bool = True):
        ...
        print(f"Loaded {model_name} with embedding dimension: {self.embed_dim}")
    ...
    def forward(self, support_images, support_labels, query_images, n_way, k_shot):
        ...
        features = self.encoder.forward_features(all_images)
...
class MetaLearner:
    def __init__(self, model, device, n_way=5, k_shot=5, q_query=5, checkpoint_dir="./checkpoints"):
...
def main():
    ...
    print("=" * 80)
    print("Few-Shot Plant Disease Classification with Prototypical Networks")
    print("=" * 80)
    print(f"Log file will be saved to: {log_file_path}")
    
    set_seed(42)
    config = {
        'n_way': 5, 'k_shot': 5, 'q_query': 5, 'epochs': 10, 
        'episodes_per_epoch': 100, 'val_episodes': 50, 'test_episodes': 1000,
        'model_name': 'vit_small_patch16_224', 'data_dir': './data',
    ...
    model = PrototypicalNetwork(config['model_name'])
